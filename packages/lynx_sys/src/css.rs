...
/// Magic bytes for the CSS token stream byte format.
pub const CSS_TOKEN_STREAM_MAGIC: [u8; 4] = *b"LCST";
/// CSS token stream byte format version.
pub const CSS_TOKEN_STREAM_VERSION: u8 = 1;
/// Header size for the CSS token stream byte format.
pub const CSS_TOKEN_STREAM_HEADER_LEN: usize = 16;
/// Per-token record size for the CSS token stream byte format.
pub const CSS_TOKEN_STREAM_RECORD_LEN: usize = 12;
...
/// Compile-time tokenized CSS stored as contiguous static bytes.
///
/// Layout:
/// - bytes 0..4: magic `LCST`
/// - byte 4: format version
/// - byte 5: header length
/// - byte 6: token record length
/// - byte 7: flags, currently zero
/// - bytes 8..12: token count, little-endian `u32`
/// - bytes 12..16: payload arena length, little-endian `u32`
/// - token records: `token_type u8`, `flags u8`, `reserved u16`,
///   `payload_offset u32`, `payload_len u32`
/// - payload arena: UTF-8 token payload bytes
...
impl CSSTokenStream {
    /// Creates a token stream from static bytes generated by `CSS!`.
    pub const fn from_static(bytes: &'static [u8]) -> Self {
        Self { bytes }
    }

    /// Returns the encoded token stream bytes.
    pub const fn as_bytes(&self) -> &'static [u8] {
        self.bytes
    }

    /// Returns a pointer to the encoded token stream bytes.
    pub const fn as_ptr(&self) -> *const u8 {
        self.bytes.as_ptr()
    }

    /// Returns the encoded token stream length.
    ...
    /// Returns true if the byte stream has the expected header.
    pub fn has_valid_header(&self) -> bool {
        self.bytes.len() >= CSS_TOKEN_STREAM_HEADER_LEN
            && self.bytes[0..4] == CSS_TOKEN_STREAM_MAGIC
            && self.bytes[4] == CSS_TOKEN_STREAM_VERSION
            && self.bytes[5] == CSS_TOKEN_STREAM_HEADER_LEN as u8
            && self.bytes[6] == CSS_TOKEN_STREAM_RECORD_LEN as u8
    }

    /// Returns the encoded token count when the header is present.
    pub fn token_count(&self) -> Option<u32> {
        read_u32_le(self.bytes, 8)
    }

    /// Returns the encoded payload arena length when the header is present.
...
mod tests {
    ...
    #[test]
    fn token_stream_reads_header() {
        static BYTES: &[u8] = &[
            b'L',
            b'C',
            b'S',
            b'T',
            CSS_TOKEN_STREAM_VERSION,
            CSS_TOKEN_STREAM_HEADER_LEN as u8,
            CSS_TOKEN_STREAM_RECORD_LEN as u8,
            0,
            2,
            0,
            0,
        ...
        let stream = CSSTokenStream::from_static(BYTES);

        assert!(stream.has_valid_header());
        assert_eq!(stream.token_count(), Some(2));
        assert_eq!(stream.payload_len(), Some(7));
    }
}
